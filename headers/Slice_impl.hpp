...
{
template <typename T>
Array<T>::Slice::Slice(arg_type begin, arg_type end, arg_type step) 
    : s_begin(begin)
    , s_end(end)
...
template <typename T>
void SamH::NumC::Array<T>::Slice::normalize(arg_type array_len)
{
    ...
    // Clamp to bounds
    if (s_begin < 0) throw std::out_of_range("Slice::Begin out of range");
    if (s_end < 0) throw std::out_of_range("Slice::End out of range");
...
template <typename T>
arg_type 
Array<T>::Slice::operator[](arg_type idx) const
{
    if (idx >= size()) {
...
template <typename T>
arg_type 
Array<T>::Slice::valid_step(arg_type step) const
{
